...
def phraseFilter(phrase, t_words,
                 f_words):  # loops through trigger words and returns true if the phrase contains a word, but not a filter word
...
def prettyPrintGames(freegames):
    for submission in freegames:
        ...
        print("Post created ",
              datetime.fromtimestamp(submission.created))  # convert UTC time when post created to something readable
...
def gimmeGames(gamedeals_min, android_gamedeals_min, free_games_android_min, ebook_deals_min):
    ...
    for submission in subreddit.hot(limit=50):
        ...
        if (score > gamedeals_min and subreddit == "gamedeals") or (
                score > android_gamedeals_min and subreddit == "androidgamedeals") or (
                score > free_games_android_min and subreddit == "freegamesonandroid") or (
                score > ebook_deals_min and subreddit == "ebookdeals"):
            # Currently using a list and having it check through them. If list gets too long, use regex or database
